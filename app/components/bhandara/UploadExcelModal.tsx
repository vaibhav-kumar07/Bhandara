...
import { bulkUploadDonations, ProcessedDonorData } from '@/actions/UploadExcel'
...
export default function UploadExcelModal({ bhandaraId, onClose }: UploadExcelModalProps) {
  ...
  const parseExcelFile = async (file: File): Promise<{ data: ProcessedDonorData[], errors: string[] }> => {
    const errors: string[] = []
    const processedData: ProcessedDonorData[] = []
    ...
    try {
      ...
      for (let i = 0; i < data.length; i++) {
        ...
        try {
          ...
          const firstName = (row['First Name'] || row['FirstName'] || row['first name'] || '').toString().trim()
          const lastName = (row['Last Name'] || row['LastName'] || row['last name'] || '').toString().trim()

          // Validate required fields
          if (!firstName || firstName.length < 2) {
            errors.push(`Row ${rowNumber}: First Name is required and must be at least 2 characters`)
            continue
          }
          ...
          let amountValue: any = row['Amount'] !== undefined ? row['Amount'] : row['amount']
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      const result = await bulkUploadDonations(processedData, bhandaraId)
      ...
      if (result.success) {
        ...
        if (result.results.failed === 0 && parseErrors.length === 0) {
          toastSuccess(`Successfully uploaded ${result.results.success} donations!`)
          setTimeout(() => {
            onClose()
            router.refresh()
          }, 2000)
        } else {
          toastSuccess(`Uploaded ${result.results.success} donations. ${result.results.failed + parseErrors.length} failed.`)
        }
      } else {
        ...
        <div className="sticky top-0 bg-white border-b border-gray-200 px-4 sm:px-6 py-4 flex items-center justify-between">
          <div>
            ...
            <p className="text-sm text-gray-600 mt-1">
              Upload donors and donations from Excel file
            </p>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="p-4 sm:p-6 space-y-5">
          ...
          <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
            ...
            <div className="text-xs text-blue-800 space-y-1">
              <p>• Column 1: <strong>Serial</strong> (optional)</p>
              <p>• Column 2: <strong>First Name</strong> (required)</p>
              <p>• Column 3: <strong>Last Name</strong> (optional - will be used as Father Name)</p>
              <p>• Column 4: <strong>Amount</strong> (required, can be 0 - if 0, no donation will be created)</p>
            </div>
            <p className="text-xs text-blue-700 mt-3 italic">
              Note: All donations will be created with default payment mode as "Cash"
            </p>
          </div>
