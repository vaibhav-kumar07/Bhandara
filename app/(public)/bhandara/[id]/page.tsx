        ...
        {bhandarastats && (
          <div className="bg-white ">
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-2 sm:gap-3 lg:gap-4">
              ...
              {/* Net Balance */}
              <div className={`bg-gradient-to-br rounded-md p-2 sm:p-3 border ${
                bhandarastats.netBalance >= 0 
                  ? 'from-purple-50 to-purple-100 border-purple-200' 
                  : 'from-orange-50 to-orange-100 border-orange-200'
              }`}>
                <div className="flex items-center gap-1 mb-1 sm:mb-2">
                  <Calculator className={`w-3 h-3 sm:w-5 sm:h-5 flex-shrink-0 ${
                    bhandarastats.netBalance >= 0 ? 'text-purple-600' : 'text-orange-600'
                  }`} />
                  <span className={`text-[10px] sm:text-sm font-medium leading-tight ${
                    bhandarastats.netBalance >= 0 ? 'text-purple-900' : 'text-orange-900'
                  }`}>Net Balance</span>
                </div>
                <p className={`text-xs sm:text-xl font-bold break-words ${
                  bhandarastats.netBalance >= 0 ? 'text-purple-900' : 'text-orange-900'
                }`}>
